...
export async function currentUser(request:Request){const token=sessionToken(request);if(!token)return null;await ensureInterclubAuthSchema();const tokenHash=await hashToken(token);const [row]=await getDb().select({id:appUsers.id,username:appUsers.username,role:appUsers.role,teamId:appUsers.teamId,preferredLanguage:appUsers.preferredLanguage,mustChangePassword:captainProfiles.mustChangePassword}).from(appSessions).innerJoin(appUsers,eq(appSessions.userId,appUsers.id)).leftJoin(captainProfiles,eq(captainProfiles.userId,appUsers.id)).where(and(eq(appSessions.tokenHash,tokenHash),gt(appSessions.expiresAt,new Date().toISOString())));return row?{...row,mustChangePassword:Boolean(row.mustChangePassword)}:null}
